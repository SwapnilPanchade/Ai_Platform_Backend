...
export const cancelSubscription = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (!user || !user.stripeSubscriptionId) {
            res.status(404).json({ message: 'Active subscription not found for this user.' });
            return;
        }
        ...
        console.log(`Subscription ${user.stripeSubscriptionId} marked to cancel at period end.`);
        // NOTE: We DO NOT update the user's role/status here.
        // The 'customer.subscription.updated' or 'customer.subscription.deleted' webhook will handle that.

        res.status(200).json({ message: 'Subscription cancellation requested successfully. Access remains until period end.', status: updatedSubscription.status });
