...
export const createUpdateUrl = async (req:Request, res:Response): Promise<void>{
    ...
    try{
        ...
        logger.info({adminUserId,upload:upload.id}, "MUX direct upload url created");
        res.status(201).json({
            uploadURL : upload.url, uploadID : upload.id
        })
        
    }catch(error:any){
        logger.error({ err: error, adminUserId }, "Failed to create Mux upload URL");
        saveLogToDb({ level: 'error', message: `Mux upload URL creation failed: ${error.message}`, error, userId: adminUserId });
